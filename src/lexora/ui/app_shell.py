...
async def attach_lexora_shell(
    ...
    *,
    ...
    Must be awaited from an ``async`` Flet session handler so startup runs on the
    same asyncio loop as the Page (avoids thread races with ``page.run_task`` when
    the sync handler runs in Flet's executor).
    """
    ...
    startup_ring = ft.ProgressRing(
        width=44,
        height=44,
        stroke_width=4,
        color=Colors.PRIMARY,
    )
    startup_title = ft.Text(
        t("app.startup.loading"),
        size=20,
        weight=ft.FontWeight.W_600,
        color=Colors.TEXT_PRIMARY,
    )
    startup_sub = ft.Text(
        t("app.startup.loading_detail"),
        size=14,
        color=Colors.TEXT_SECONDARY,
        text_align=ft.TextAlign.CENTER,
        width=420,
    )
    startup_layout = ft.Container(
        expand=True,
    ...
    page.add(startup_layout)
    page.update()

    _log = logging.getLogger("lexora.ui.shell")

    async def _bootstrap() -> None:
        try:
            # Yield so the host can paint the loading state before heavy sync work.
            ...
            _log.exception("Lexora UI startup failed while loading local data and screens")
            startup_ring.visible = False
            startup_layout.content = ft.Column(
                [
