...
- ✅ **FIXED**: These errors are now automatically handled with a workaround
- Some EPUB files have incomplete table of contents metadata (TOC items with `uid=None`)
- Lexora now detects this issue, auto-generates missing UIDs, and retries the EPUB write
- Previously failed files like `israelsailing.epub` now translate successfully
- See [TRANSLATION_FIX_LOG.md](TRANSLATION_FIX_LOG.md) for implementation details
...
4. **israelsailing.epub** - BIDI and Hebrew Text
   ...
   - Status: ✅ Now works perfectly - TOC error is automatically handled
